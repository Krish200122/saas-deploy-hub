...
if (!empty($postdata)) {
    ...
    $productId = $request['prdId'];
    $variant_id = $request['variantId'];
    $productName = $request['prdName'];
    $catId = $request['catId'];
 ...
 foreach ($request['Uploaded_images'] as $imageName => $imageData) {
    ...
    $additionalViewThumbnail = $thumbnailPaths['Additional_View'] ?? '';
     ...
     if ($productId === 0) {
            ...
            $sql = "INSERT INTO tblproducts(prdId,prdName,prdPrice,prdMrp,prdsubcatId,prdDescription,fvoriginal,bvoriginal,CGST,SGST,HSN,status) VALUES($prd_Id,'$productName',$prdPrice,$mrpPrice,$subCatId,'$prdDescription','$frontViewOriginal','$backViewOriginal',$CGST,$SGST,$HSN,'$status')";
            if ($con->query($sql) == TRUE) {
                ...
                $sql1 = "INSERT INTO tblvariantproduct(variantId,varprdId,varcolorId,varsizeId,varprdPrice,varprdMrp,prdDiscount,stock,fvoriginal,bvoriginal,tvoriginal,svoriginal,avoriginal,fvthumbnail,bvthumbnail,tvthumbnail,svthumbnail,avthumbnail,prd_status) VALUES($varient_Id,$prd_Id,$colorId,$sizeId,$prdPrice,$mrpPrice,$prdDiscount,$stock,'$frontViewOriginal','$backViewOriginal','$topViewOriginal','$sideViewOriginal','$additionalViewOriginal','$frontViewThumbnail','$backViewThumbnail','$topViewThumbnail','$sideViewThumbnail','$additionalViewThumbnail',true)";
                
                if ($con->query($sql1) == TRUE) {
                  
                $response[] = array('sts' => true, 'msg' => 'product and varient product Successfully uploaded');
                }else {

                 $response[] = array('sts' => false, 'msg' => 'Error in query: ' . $con->error);
              ...
              if ($con->query($sql2) == TRUE) {
                $sql3 = "UPDATE tblvariantproduct SET varprdId='$productId',varcolorId='$colorId',varsizeId='$sizeId',varprdPrice='$prdPrice',varprdMrp='$mrpPrice',prdDiscount='$prdDiscount',stock=$stock,fvoriginal='$frontViewOriginal',bvoriginal='$backViewOriginal',tvoriginal='$topViewOriginal',svoriginal='$sideViewOriginal',avoriginal='$additionalViewOriginal',fvthumbnail='$frontViewThumbnail',bvthumbnail='$backViewThumbnail',tvthumbnail='$topViewThumbnail',svthumbnail='$sideViewThumbnail',avthumbnail='$additionalViewThumbnail',prd_status='$status' , vartaxPrice = '$taxAmount' WHERE variantId = '$variant_id'";
                
                
                if ($con->query($sql3) == TRUE) {
                 $response[] = array('sts' => true, 'msg' => 'product and varient product successfully updated');
                }else {

                $response[] = array('sts' => false, 'msg' => 'error in product update'. $con->error);
            ...
            $sql2 = "UPDATE tblproducts SET prdName='$productName',prdPrice=$prdPrice,prdMrp = $mrpPrice,prdDescription ='$prdDescription',prdsubcatId =$subCatId,CGST=$CGST,SGST=$SGST,HSN=$HSN,status='$status' WHERE prdId=$productId";
             if ($con->query($sql2) == TRUE) {
                $sql3 = "UPDATE tblvariantproduct SET varprdId=$productId,varcolorId=$colorId,varsizeId=$sizeId,varprdPrice=$prdPrice,varprdMrp=$mrpPrice,prdDiscount=$prdDiscount,stock=$stock,prd_status='$status', vartaxPrice = '$taxAmount' WHERE variantId = $variant_id";
                
                
                if ($con->query($sql3) == TRUE) {
                 $response[] = array('sts' => true, 'msg' => 'products and varient product successfully updated');
                }else {
                ...
                $sql = "SELECT prdId FROM tblproducts Where prdName='$productName'";
                ...
                if($variant_id == null)
                {
                    ...
                    if($fvo){
                        $sql1 = "INSERT INTO tblvariantproduct(variantId,varprdId,varcolorId,varsizeId,varprdPrice,varprdMrp,prdDiscount,stock,prd_status,fvoriginal,bvoriginal,tvoriginal,svoriginal,avoriginal,fvthumbnail,bvthumbnail,tvthumbnail,svthumbnail,avthumbnail) VALUES($varient_Id,$prd_id,$colorId,$sizeId,$prdPrice,$mrpPrice,$prdDiscount,$stock,true,'$fvo','$bvo','$vto','$svo','$avo','$fvt','$bvt','$tvt','$svt','$avt')";
                    ...
                    $response[] = array('sts' => true, 'msg' => 'varient product Successfully uploaded');
                    }else {
    
                     $response[] = array('sts' => false, 'msg' => 'Error in varient product query: ' . $con->error);
    
                    }
                }else{
                    if($request['Uploaded_images']){
                        
                       $sql = "UPDATE tblvariantproduct SET varprdId =$prd_id, varcolorId= $colorId, varsizeId = $sizeId,varprdPrice =$prdPrice,varprdMrp = $mrpPrice,prdDiscount = $prdDiscount,stock = $stock,fvoriginal = '$frontViewOriginal', bvoriginal = '$backViewOriginal', tvoriginal= '$topViewOriginal', svoriginal = '$sideViewOriginal', avoriginal = '$additionalViewOriginal', fvthumbnail = '$frontViewThumbnail', bvthumbnail = '$backViewThumbnail', tvthumbnail = '$topViewThumbnail', svthumbnail = '$sideViewThumbnail', avthumbnail ='$additionalViewThumbnail', vartaxPrice = '$taxAmount' WHERE variantId = $variant_id";
                      
                    }else{
                        $sql = "UPDATE tblvariantproduct SET varprdId =$prd_id, varcolorId= $colorId, varsizeId = $sizeId,varprdPrice =$prdPrice,varprdMrp = $mrpPrice,prdDiscount = $prdDiscount,stock = $stock, vartaxPrice = '$taxAmount' WHERE variantId = $variant_id";
                       
                    }
                    if ($con->query($sql) == TRUE) {
                        
                    $response[] = array('sts' => true, 'msg' => 'varient product Successfully updated');
                    
                    }else {
    
                    $response[] = array('sts' => false, 'msg' => 'error occured while update');
